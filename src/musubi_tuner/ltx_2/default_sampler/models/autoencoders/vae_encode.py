...
from .causal_video_autoencoder import (
    CausalVideoAutoencoder,
)
from .video_autoencoder import (
    Downsample3D,
    VideoAutoencoder,
)
...
def get_vae_size_scale_factor(vae: AutoencoderKL) -> float:
    # Handle CausalVideoAutoencoder and LTX-2 VAE which have scale factors directly
    if isinstance(vae, CausalVideoAutoencoder) or hasattr(vae, 'spatial_downscale_factor'):
        ...
        spatial = vae.spatial_downsample_factor
        temporal = vae.temporal_downsample_factor
    else:
        down_blocks = len(
            [
                block
